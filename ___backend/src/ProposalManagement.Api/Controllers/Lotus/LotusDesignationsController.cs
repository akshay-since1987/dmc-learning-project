...
[ApiController]
[Route("api/lotus/designations")]
[Authorize(Roles = "Lotus")]
public class LotusDesignationsController : ControllerBase
{
    private readonly IMediator _mediator;
    public LotusDesignationsController(IMediator mediator) => _mediator = mediator;
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateDesignationCommand command, CancellationToken ct = default)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateDesignationCommand command, CancellationToken ct = default)
    {
    ...
    {
        var result = await _mediator.Send(new DeleteDesignationCommand(id), ct);
        return result.IsSuccess ? NoContent() : StatusCode(result.StatusCode, new { result.Error });
    }
}
