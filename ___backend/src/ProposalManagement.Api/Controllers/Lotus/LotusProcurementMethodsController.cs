...
[ApiController]
[Route("api/lotus/procurement-methods")]
[Authorize(Roles = "Lotus")]
public class LotusProcurementMethodsController : ControllerBase
{
    private readonly IMediator _mediator;
    public LotusProcurementMethodsController(IMediator mediator) => _mediator = mediator;
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateProcurementMethodCommand command, CancellationToken ct = default)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateProcurementMethodCommand command, CancellationToken ct = default)
    {
    ...
    {
        var result = await _mediator.Send(new DeleteProcurementMethodCommand(id), ct);
        return result.IsSuccess ? NoContent() : StatusCode(result.StatusCode, new { result.Error });
    }
}
