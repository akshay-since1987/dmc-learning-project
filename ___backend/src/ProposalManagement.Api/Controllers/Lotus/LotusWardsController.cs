...
[ApiController]
[Route("api/lotus/wards")]
[Authorize(Roles = "Lotus")]
public class LotusWardsController : ControllerBase
{
    private readonly IMediator _mediator;
    public LotusWardsController(IMediator mediator) => _mediator = mediator;
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateWardCommand command, CancellationToken ct = default)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateWardCommand command, CancellationToken ct = default)
    {
    ...
    {
        var result = await _mediator.Send(new DeleteWardCommand(id), ct);
        return result.IsSuccess ? NoContent() : StatusCode(result.StatusCode, new { result.Error });
    }
}
