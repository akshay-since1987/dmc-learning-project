...
public class GetDesignationsQueryHandler : IRequestHandler<GetDesignationsQuery, PagedResult<MasterListItemDto>>
{
    private readonly IRepository<Designation> _repo;

    public GetDesignationsQueryHandler(IRepository<Designation> repo)
    {
        _repo = repo;
    }

    public async Task<PagedResult<MasterListItemDto>> Handle(GetDesignationsQuery request, CancellationToken cancellationToken)
    {
